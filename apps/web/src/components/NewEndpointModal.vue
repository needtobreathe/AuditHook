...
<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 shadow-2xl space-y-4">
      ...
      <div v-if="error" class="p-2.5 rounded bg-rose-950/50 border border-rose-800/80 text-rose-300 text-xs">
        {{ error }}
      </div>

      <div class="space-y-3 text-xs">
        <div>
          <label class="block text-slate-300 font-medium mb-1">Endpoint Name</label>
          <input
            v-model="name"
            type="text"
            placeholder="e.g. Stripe Payment Listener"
            class="w-full bg-slate-950 border border-slate-800 rounded-md px-3 py-2 text-white focus:outline-none focus:border-indigo-500 font-sans"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            class="w-full bg-slate-950 border border-slate-800 rounded-md px-3 py-2 text-white focus:outline-none focus:border-indigo-500 font-mono"
          />
        </div>

        <div>
          <label class="block text-slate-300 font-medium mb-1">Target Localhost URL</label>
          <input
            v-model="targetUrl"
            type="text"
        ...
        <div class="pt-2 flex items-center space-x-2">
          ...
          />
          <label for="autoForward" class="text-slate-300 cursor-pointer">
      ...
      <div class="flex items-center justify-end space-x-3 pt-3 border-t border-slate-800">
        <button
          @click="emit('close')"
          class="px-3 py-2 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium"
        >
          Cancel
        </button>
        <button
